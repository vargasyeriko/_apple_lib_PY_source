...
#########################
    ...
    sizes_financial = [100, 300]  # Example values for 'final_cash' and 'final_dividends'
    colors_financial = ['limegreen', 'peachpuff']
    ...
    textprops_financial = {'fontsize': 8, 'fontfamily': 'Arial', 'fontweight': 'normal'}
    
    # Create a figure with a specific size
    plt.figure(figsize=(8, 6))
    
    # Define the axes for the pie chart, position it at the bottom-right corner
    # `left` is set close to the right edge minus the width, `bottom` is close to 0
    ax_financial = plt.axes([0.6, 0.1, 0.3, 0.3])  # Adjust the values as needed to fit within your figure
    
    # Create the pie chart
    wedges, texts, autotexts = ax_financial.pie(sizes_financial, explode=explode_financial, labels=labels_financial, colors=colors_financial, 
                                                autopct=autopct_financial, shadow=shadow_financial, startangle=startangle_financial, textprops=textprops_financial)
    
    # Equal aspect ratio ensures that pie is drawn as a circle
    ax_financial.axis('equal')
