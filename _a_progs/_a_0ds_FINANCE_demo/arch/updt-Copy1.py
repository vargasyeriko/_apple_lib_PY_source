...
for i in range(len(original_df_add)):
    ...
    df_cash_ledger = df_cash_ledger.append(df_stock_to_cash_ledger, ignore_index=True).sort_values(by='DateTrans').reset_index(drop=True)  
...
# if start_done == 'yes':
#     # Step 1: Remove rows where 'NewColumn' is 0
#     df = df[df['NewColumn'] != 0]
#     # Step 2: Drop the 'NewColumn' from the DataFrame
...
#input('')
#df_add_original_lenght
 # this is being used for names of Cash excess
# ############################################################################
# ############################################################################
# **********************_1_ Ledger ***************************** ')
# ############################################################################
...
df_cash_bal = df_cash_bal.append(df_cash_init, ignore_index=True).sort_values(by='DateTrans').reset_index(drop=True)   
    
# ############################################################################ COCNAT (1)(2) & 3
#to the empty one, so thats your up to ledger df
df_cash_bal = df_cash_bal.append(df_cash_init_empty, ignore_index=True).sort_values(by='DateTrans').reset_index(drop=True)   
...
#print('-_-_-_-_-_-_-_-_-_-_-_HOLD_UNITS_UPDATED-_-_-_-_-_-_-_-_-_' )
#print('AFTER Cash Excess update hold_units_at_init *** status : updated \n ')
# ############################################################################
#from _1_a_CASH_Excess_ import filter_df_by_date
# ############################################################################
exec(open(f"_1_a_CASH_Excess_.py",encoding="utf-8").read()) #paths
...
if filter_cash_excess_check != 0:
    ...
    #print('**********************************************************')
    #print(f'NOW processing cash excess for \n\n{df_cash_x_stock_ads}')
    ...
    for index, row in df_cash_x_stock_ads.iterrows():
        ...
        df_master_unit_updt_inv_init = pd.read_pickle('_1_tables/_df_01_init_INVESTMENT_UNITS_START_.pkl')


        # Create a template for new rows with all other columns set to 0
        new_row_template = {col: 0 for col in df_master_unit_updt_inv_init.columns}
        ...
        for index, row in df_stocks_add.iterrows():
            if row['Stock'] not in df_master_unit_updt_inv_init['Stock'].values:
                ...
                df_master_unit_updt_inv_init = df_master_unit_updt_inv_init.append(new_row_template, ignore_index=True)

        # Optionally, save the updated DataFrame back to a pickle file
        df_master_unit_updt_inv_init.to_pickle('_1_tables/_df_01_init_INVESTMENT_UNITS_START_.pkl')
...
# # ############################################################################
# INITIALIZE EXCESS if needed 
# # ############################################################################
# # **********************_1_ CASH and DIV ***************************** ')

def update_or_append(df_master, df_add):
    # Iterate over the rows of df_stocks_add
    for index, row in df_add.iterrows():
        if row['Stock'] in df_master['Stock'].values:
            # Update existing stock units
            df_master.loc[df_master['Stock'] == row['Stock'], 'hold_units_at_init'] += row['units_acquired']
        else:
            # Append new stock
            new_data = {
                'Stock': row['Stock'],
                'hold_units_at_init': row['units_acquired'],
                'StartDate': row['DateTrans']
            }
            df_master = df_master.append(new_data, ignore_index=True)
    return df_master

# Apply the function
df_master_unit_updt = update_or_append(df_master_unit_updt, df_stocks_add)
# # ############################################################################
# # ############################################################################
# UPDATE UNITS FROM CASH EXCESS
df_master_unit_updt.to_pickle(f'_1_tables/{df_name_units_updt}.pkl')
#df_master_unit_updt.to_pickle(f'_1_tables/_test_1a_{df_name_units_updt}.pkl')
# # ############################################################################
# # ############################################################################

# UPDATE LADGER FROM CASH EXCESS
#exec(open(f"_1_a_CASH_Excess_.py",encoding="utf-8").read()) #paths
#df_div.to_pickle(f'_1_tables/_test_3_df_div_2_up_to_date_LEDGER.pkl')
# # ############################################################################
# # ############################################################################
