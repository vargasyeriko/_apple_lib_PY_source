...
def ds_cv():
    ...
    this just shows how I am able to break apart big challenges. Also at ///pos_added/// [[[I ///poss_added_desc///  ]]] 
...
# 4 --<<EXPORTS>>
    ...
    # # Function to find the first matching keyword in each text variable
    # def find_and_save_text(texts, keywords):
    #     for text in texts:
    #         # Find the first keyword that appears in the text
    #         found_code = next((keyword for keyword in keywords if keyword in text), None)
    #         if found_code:
    #             # Define the filename based on the found code
    #             filename = f"{path_tmep_j_desc}/{found_code}_custom_task_desk.txt"
    #             # Save the text to a file with the filename based on the found code
    #             with open(filename, 'w') as file:
    #                 file.write(text)
    #             print(f"File saved: {filename}")
    
    # # Assuming the actual text content for cai_desc_this, ford_desc_this, and added_desc_this are set, run the function
    # # This is a demonstration, so the actual content will need to be replaced with the variables' contents
    # find_and_save_text(text_variables, keywords)
