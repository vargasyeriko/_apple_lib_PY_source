...
def _filefinder_by_EXT_GET_df(df, file_extensions):
    """
    This function analyzes paths from an existing DataFrame['Path'] for all specified music file extensions (case insensitive),
    or, if no file extensions are provided, it will return all files.
    
    Filters out unwanted files (starting with '._' and '.DS'), and returns a DataFrame containing the file path,
    the base name without extension ('Name'), the full file name ('file_name'), and the size in MB.
    
    Includes a progress tracker (TQM) and handles mixed case extensions.

    Parameters:
    - df (pd.DataFrame): Existing DataFrame that contains a column named 'Path'.
    - file_extensions (list, optional): A list of file extensions to include (e.g., ['.wav', '.aiff', '.mp3']).
    
    Returns:
    - df (pd.DataFrame): A new DataFrame with filtered file info.
    - fre_tab (pd.DataFrame): Frequency table by extension.
    """
    
    df_out = pd.DataFrame(columns=['Path', 'Name', 'file_name', 'Size (MB)', 'Extension'])

    if 'Path' not in df.columns:
        print("Error: Input DataFrame must contain a 'Path' column.")
    ...
    try:
        with tqdm(total=total_files, desc="🔍 Verifying Paths", unit="files") as pbar:
            for path in df['Path']:
                ...
                if not file_extensions or ext in file_extensions:
                    try:
                        ...
                        print(f"Skipping missing file: {path}, reason: {e}")
                pbar.update(1)

        if file_list:
            df_out = pd.DataFrame(file_list)

    except Exception as e:
        print(f"Unexpected error: {e}")

    df_out = df_out[~df_out['file_name'].str.startswith('._')].copy()
    df_out = df_out[~df_out['file_name'].str.startswith('.DS')].copy()

    if not df_out.empty:
        fre_tab = df_out.groupby('Extension').agg(frequency=('Extension', 'size'),
                                                  total_size_in_mb=('Size (MB)', 'sum')).reset_index()
        grand_total_size_mb = df_out['Size (MB)'].sum()
    ...
    print(f"ATTN ::: <E>  AFTER making sure that you have a column ['Path'] in your df")
    print(f"***df*** will be returned having ::: {len(df_out)}  rows, write df in next cell to see your DATA FRAME\n")
