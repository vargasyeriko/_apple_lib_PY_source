...
import subprocess  # for opening the image

# -----######-----###### MAIN FUNCTION #####-----######-----

def _kick_0911_story_GET_dashboard_png(path_in, out_png_path=None, dpi=220):
    """
    Build a visual dashboard ("story") for a single kick sample and save it as a PNG,
    then automatically open it with the system viewer (Preview on macOS).
    """
    # ---------------- TQM HELPER ----------------
    ...
    if attack_idx is not None:
        decay_idx = next((i for i in range(attack_idx, len(env)) if env[i] <= decay_thr), None)
    ...
    if attack_idx: ax_wave.axvline(attack_idx / sr, color="red", linestyle="--", label="Attack")
    if decay_idx: ax_wave.axvline(decay_idx / sr, color="orange", linestyle="--", label="Decay")
    ...
    if f0_hz and f0_hz >= 20:
        ax_spec.axvline(f0_hz, color="red", linestyle="--")
        ax_spec.text(f0_hz, np.max(mag_db[valid]), f"f0 ≈ {f0_hz:.1f} Hz", color="red", fontsize=8, va="bottom")

    # Spectrogram
    ax_sgram = axes[1, 0]
    img = librosa.display.specshow(S_db, sr=sr, hop_length=hop_length, x_axis="time", y_axis="hz", ax=ax_sgram, cmap="magma")
    ...
    if attack_idx: ax_env.axvline(attack_idx / sr, color="red", linestyle="--")
    if decay_idx: ax_env.axvline(decay_idx / sr, color="orange", linestyle="--")
    ...
    stats_text = "\n".join([
        ...
        f"Attack: {attack_ms:.1f} ms" if attack_ms else "",
        f"Decay: {decay_ms:.1f} ms" if decay_ms else "",
        f"f0: {f0_hz:.1f} Hz" if f0_hz else "",
    ])
    ax_env.text(0.02, 0.98, stats_text, transform=ax_env.transAxes,
    ...
    subprocess.run(["open", out_png_path])
    print(f"[SUCCESS] Kick dashboard saved & opened -> {out_png_path}")
    return out_png_path
