...
def _bpm_2610_i3_GET_partialdonut_bpmdynamic(df: pd.DataFrame, save_dir: str = "images") -> None:
    ...
    #color_bpm_circle = "#A8D8E8"
    color_mapping = {
    'D_0': "#FFEB3B",  # Bright neon yellow (high energy)
    'D_1': "#E0C933",  # Slightly dimmer, golden hue
    'D_3': "#B89F2A",  # Muted yellow, earthy tone
    'D_6': "#8F771F",  # More brown, low vibrance
    'D_9': "#5C4F14"   # Almost mustard-brown, dark
    }
...
color_luf_circle = '#FFFFFF'
lufs_color_map = {
    'LuA': color_luf_circle,  # Light pink red
    'LuB': color_luf_circle,  # Soft cherry
    'LuC': color_luf_circle,  # Vibrant rose
    'LuD': color_luf_circle,  # Deep raspberry
    'LuE': color_luf_circle,  # Scarlet red
    'LuF': color_luf_circle,  # Crimson red
}
...
def _lufs_1504_i1_GET_partialdonut_lufs(df, save_dir="images_lufs"):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        donut_color = lufs_color_map.get(lufs_cat, color_luf_circle)
    ...
    for path in tqdm(df['Path'], desc="TQM: Spectral Shape-Texture Mapping"):
        try:
            ...
            if sf < 0.09:
                # Tonal block
                if sb < 2000:
                    hex_code = '#ADD8FF'   # Subby / low focus
                elif sb < 3000:
                    hex_code = '#3EA4FF'   # Lower groove zone
                elif sb < 4200:
                    hex_code = '#005EFF'   # Core groove / mix body
                elif sb < 5200:
                    hex_code = '#113C70'   # Highs / tribal hats
                else:
                    hex_code = '#000B2D'   # Sparkly shimmer
            else:
                # Noisy block
                if sb < 2000:
                    hex_code = '#FFE8CF'   # Soft noise
                elif sb < 3000:
                    hex_code = '#FFC78C'   # Noisy groove
                elif sb < 4200:
                    hex_code = '#FFA64F'   # Brighter distortion
                elif sb < 5200:
                    hex_code = '#FF7600'   # High-end crackle
                else:
                    hex_code = '#C84200'   # Harsh sparkle
...
# -----######-----######-----######-----######-----######-----#
# 0_FNS: _centroid_1604_i2_GET_donutfade_centroidcolor
# -----######-----######-----######-----######-----######-----#
...
def _centroid_1604_i2_GET_donutfade_centroidcolor(df, save_dir="images_centroid/"):
    """
    Creates an ultra-intense donut image with only a light fade at the edge.
    Donut is strong and colorful. Saves as 'centroid_donut_{ID}.png' per row.
    """
    ...
    path_list = []
    print("TQM: Generating ultra-intense centroid color donuts...")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        fade_band = 4  # <-- Only last few pixels will fade
        ...
        for r in range(radius_inner, radius_outer):
            if r >= radius_outer - fade_band:
                alpha = 1.0 - (r - (radius_outer - fade_band)) / fade_band
            else:
                alpha = 1.0
    ...
    print("TQM: Intense donuts saved — pure saturation with just a tiny fade.")
...
def _keyid_1604_i3_GET_png_label_blocks_rightflush(df, save_dir='img_keyid'):
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            year_suffix = f"{date_val.year}" if pd.notnull(date_val) else "0000"
        except:
            year_suffix = "0000"
        ...
        ax.text(
            0.98, 0.45, display_id,  # moved a little higher
            fontsize=46,
            ha='right', va='center', color='black'
        )
